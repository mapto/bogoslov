...
def test_find_case():
    ...
    assert result == [('ъсѣ землѣ да поклоні-т-і сѩ і поетъ теб', '//none.txt#1', 0.75,)]

@fixture
def test_find_nocase():
    from app_lcs import find
    result = find([],"въса землꙗ да поклонит ти се и поеть тебе", match_case=False)
    assert result == [('въсѣ землѣ да поклоні-т-і сѩ і поетъ теб', '//none.txt#1', 0.7654320987654321,)]    