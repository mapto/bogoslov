...
echo "DB container needs to be started"
...
# lat
declare -a models=("LaBSE" "multilingual-e5-base" "colbert-xm" "SPhilBerta" "paraphrase-multilingual-mpnet-base-v2")

printf -v date '%(%Y%m%d)T' -1
echo "Date is $date"

echo "Creating 1-verses-$date.sql"
docker exec bogoslov_$corpus_lang-db-1 pg_dump -Ubogoslov -tverses bogoslov -a > 1-verses-$date.sql
gzip 1-verses-$date.sql

echo "Creating 2-ngrams-$date.sql"
docker exec bogoslov_$corpus_lang-db-1 pg_dump -Ubogoslov -tngrams bogoslov -a > 2-ngrams-$date.sql
gzip 2-ngrams-$date.sql

docker exec bogoslov_$corpus_lang-db-1 pg_dump -Ubogoslov -tembeddings bogoslov -a > 3-embeddings-$date.sql
